...
#include <provision/internal/ucx/pvi_ucx_mac_mgr.h>
  ...
  bool                                          warmboot)
{
  ...
  available_speed_masks =
    apr_pcalloc(driver_instance->subpool,
  ...
  driver_data =
    apr_pcalloc(driver_instance->subpool,
                sizeof(struct pvi_network_ports_driver_data));
  ...
  struct pvi_network_ports_mgr_resources *resources)
{
  ...
  resources->ports_number    = FRONT_PLANE_PORTS;
  resources->fp_ports_number = FRONT_PLANE_PORTS;
  ...
  struct pv_network_ports_status *status)
{
  ...
  if (FRONT_PLANE_PORTS > port_id) {
    ...
    /*
     * Check whether the port is type GIG, XG, or 40GIG in order to get the
     * correct
     * register address
     */

    if (status->speed == PV_PORT_SPEED_10G);
    else if ();
